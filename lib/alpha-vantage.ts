...
export async function fetchAllMarketData() {
  type RegionKey = keyof typeof MARKET_REGIONS;
  type MarketRegionRow = ReturnType<typeof generateFallbackData>;

  const result: {
    americas: MarketRegionRow[];
    emea: MarketRegionRow[];
    asiaPacific: MarketRegionRow[];
    lastUpdated: string;
  ...
  for (const [region, ids] of Object.entries(MARKET_REGIONS) as Array<
    [RegionKey, (typeof MARKET_REGIONS)[RegionKey]]
  >) {
